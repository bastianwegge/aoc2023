...
func process(input string) int {
	...
	searchParams := Bag{r: 12, g: 13, b: 14}
	foundGames := make([]int, 0)
	for gameIndex, bag := range gameToBag {
		if bag.r <= searchParams.r && bag.g <= searchParams.g && bag.b <= searchParams.b {
			foundGames = append(foundGames, gameIndex)
		}
	}

	fmt.Println(foundGames)

	sum := 0
	for _, gameIndex := range foundGames {
		sum += gameIndex
	}
